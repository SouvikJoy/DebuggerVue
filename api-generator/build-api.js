...
const libraryName = "DebuggerVue";
const libraryVersion = pkg.version;

const showcaseURL = "https://github.com/SouvikJoy/studious-octo-tribble.git";
...
const createWebTypes = (component) => {
  const url =
    showcaseURL +
    `${
      component["doc-url"] ? component["doc-url"] : component.name.toLowerCase()
    }`;
  ...
  if (component["vue-modifiers"]) {
    ...
    attribute["vue-modifiers"] = addURL(
      component["vue-modifiers"],
      url,
      "name"
    );
...
fs.writeFileSync(
  path.resolve(distDir, "vetur-attributes.json"),
  veturAttributesJson
);
